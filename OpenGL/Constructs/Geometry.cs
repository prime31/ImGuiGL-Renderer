...
namespace OpenGL
{
    ...
    public class Geometry
    {
        ...
        {
            Vector3[] vertices = { new Vector3(location.X, location.Y, 0), new Vector3(location.X + size.X, location.Y, 0), 
        ...
        public static VAO CreateQuad(ShaderProgram program, Vector2 location, Vector2 size, Vector2 uvloc, Vector2 uvsize)
        {
            Vector3[] vertices = { new Vector3(location.X, location.Y, 0), new Vector3(location.X + size.X, location.Y, 0), 
                new Vector3(location.X + size.X, location.Y + size.Y, 0), new Vector3(location.X, location.Y + size.Y, 0) };
            Vector2[] uvs = { uvloc, new Vector2(uvloc.X + uvsize.X, uvloc.Y), new Vector2(uvloc.X + uvsize.X, uvloc.Y + uvsize.Y), new Vector2(uvloc.X, uvloc.Y + uvsize.Y) };
            uint[] indices = { 0, 1, 2, 2, 3, 0 };
